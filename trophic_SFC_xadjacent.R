...
# trophic SFC spatially explicit model
...
mu2 <- 0.45       # probablilty of consumer extinction due to predator presence
psi1 <- 0.4       # probablilty of consumer extinction due lack of resource
psi2 <- 0.4       # probablilty of predator extinction due lack of consumer
...
dynamicsD <- function(cx, cy, cz, ex, ey, ez, mu1, mu2, psi1, psi2, px0, py0, pz0, n, tmax, dD) {
  ...
  EY2 <- c((ey+psi1+mu2), (ey+mu2), (1-ey-psi1-mu2), (ey+psi1+mu2), (1-ey-mu2), (ey+mu2), (1-ey-psi1-mu2), (1-ey-mu2))
  EZ  <- c((ez+psi2), (ez+psi2), ez, (1-ez-psi2), ez, (1-ez-psi2), (1-ez), (1-ez))
    ...
    for(g in 2:length(t)){
      ...
      for(i in 1:n){
        for(j in 1:n){
          ...
          p70 <- EX2[1] * EY2[1] * EZ[1]
          p71 <- EX2[2] * EY2[2] * EZ[2]
          p72 <- EX2[3] * EY2[3] * EZ[3]
          p73 <- EX2[4] * EY2[4] * EZ[4]
          p74 <- EX2[5] * EY2[5] * EZ[5]
          p75 <- EX2[6] * EY2[6] * EZ[6]
          p76 <- EX2[7] * EY2[7] * EZ[7]
          p77 <- EX2[8] * EY2[8] * EZ[8]
...
outD <- dynamicsD(cx, cy, cz, ex, ey, ez, mu1, mu2, psi1, psi2, px0, py0, pz0, n, tmax, dD)
...
dynamicsR <- function(cx, cy, cz, ex, ey, ez, mu1, mu2, psi1, psi2, tmax, DR, dR) {
  ...
  for(k in 2:length(r)){
      ...
      for(j in 1:n){
        ...
        if(xreq[i,j,k]!=-1) { next }
          ...
          for(h in 1:4){
            ...
            if(i_n < 1 || i_n > n || j_n < 1 || j_n > n) { next } # neighbour outside of grid
            if(xreq[i_n,j_n,k]==1 || xreq[i_n,j_n,k]==4 || xreq[i_n,j_n,k]==5 || xreq[i_n,j_n,k]==7) {
              n_x <- n_x+1
              d_x[n_x,1] <- i
              d_x[n_x,2] <- j
              n1 <- 1
              break }               # if neighbour occupied by x, move to next patch
            else if(xreq[i_n,j_n,k]==2 || xreq[i_n,j_n,k]==6) {
              n2 <- 1 }
            else if(xreq[i_n,j_n,k]==3) {
              n3 <- 1 }
          }
          if(n1==0 && n2==1) {
            n_y <- n_y+1
    ...
    # restore patches adjecent to x-occupied patches first, then y-occupied, then z-occupied, then empty
    if(n_x<dn) {
      if(n_x>0) {
        for(h in 1:n_x) {
          i_d <- d_x[h,1]
      ...
      if(n_y>=(dn-n_x)) {
        ...
        if(n_z>=(dn-n_x-n_y)) {
          ...
          rp <- sample(1:n_e,(dn-n_x-n_y-n_z))
          for(h in 1:(dn-n_x-n_y-n_z)) {
            i_d <- d_e[rp[h],1]
            j_d <- d_e[rp[h],2]
            xreq[i_d,j_d,k] <- 0
          }
...
outR <- dynamicsR(cx, cy, cz, ex, ey, ez, mu1, mu2, psi1, psi2,  tmax, DR, dR)
