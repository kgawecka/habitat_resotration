...
# pairwise mutualism spatially explicit model
# non-random habitat resotration - x-adjacent method
#                                - sites adjecent to occupied by plant restored first, 
#                                  then sites adjecent to occupied by animal restored, 
#                                  then empty restored
...
# patch values:
# 0 - empty
# 1 - occupied by x only
# 2 - occupied by y only
# -1 - destroyed
...
cx <- 0.4         # probablity of colonisation - plant
ex <- 0.2         # probability of extinction - plant
cy <- 0.4         # probablity of colonisation - animal
ey <- 0.2         # probability of extinction - animal
px0 <- 0.9        # initial fraction of occupied sites - plant
py0 <- 0.1        # initial fraction of occupied sites - animal
n <- 100          # grid size
...
dynamicsD <- function(cx, cy, ex, ey, px0, py0, n, tmax, dD) {
  
  t <- 0:tmax
  pnx0 <- round(px0*n*n, digits=0)   # initial number of occupied sites - plant
  if(pnx0<1) { return("ERROR: initial number of plant occupied sites = 0") }
  pny0 <- round(py0*n*n, digits=0)   # initial number of occupied sites - animal
  if(pny0<1) { return("ERROR: initial number of animal occupied sites = 0") }
  ...
  pxdt[1,1] <- (sum(xt[ , , 1]==1)+sum(xt[ , , 1]==2))/(n*n)
  ...
  for(k in 1:length(d)){
    ...
    if(k>1){
      ...
      # grid at t=0 for d[k]
      xt[ , ,1] <- xdeq[ , ,k]
      pxdt[k,1] <- (sum(xt[ , , 1]==1)+sum(xt[ , , 1]==2))/(n*n)
      Pxdt[k,1] <- (sum(xt[ , , 1]==1)+sum(xt[ , , 1]==2))/(n*n-d[k])
      pydt[k,1] <- sum(xt[ , , 1]==2)/(n*n)
      Pydt[k,1] <- sum(xt[ , , 1]==2)/(n*n-d[k])
    }
      ...
      for(i in 1:n){
        for(j in 1:n){
          ...
          for(h in 1:4){
            ...
            if (xt[i_n,j_n,g-1]==2) { ny <- ny + 1 }
          }
          
          # transition probabilities
          p00 <- (1-cx)^ny
          p01 <- (1-(1-cx)^ny) * (1-cy)^ny
          p02 <- (1-(1-cx)^ny) * (1-(1-cy)^ny)
          ...
          else if(xt[i,j,g-1]==0) { xt[i,j,g] <- sample(c(0,1,2),1, prob=c(p00, p01, p02)) }
          
          else if(xt[i,j,g-1]==1) { xt[i,j,g] <- sample(c(0,1,2),1, prob=c(p10, p11, p12)) }
          
          else if(xt[i,j,g-1]==2) { xt[i,j,g] <- sample(c(0,1,2),1, prob=c(p20, p21, p22)) }
        }
      }
      pxdt[k,g] <- (sum(xt[ , , g]==1)+sum(xt[ , , g]==2))/(n*n)
      Pxdt[k,g] <- (sum(xt[ , , g]==1)+sum(xt[ , , g]==2))/(n*n-d[k])
      pydt[k,g] <- sum(xt[ , , g]==2)/(n*n)
      Pydt[k,g] <- sum(xt[ , , g]==2)/(n*n-d[k])
    }
    xdeq[ , ,k] <- xt[ , ,length(t)]
  }
...
outD <- dynamicsD(cx, cy, ex, ey, px0, py0, n, tmax, dD)  # habitat destruction output
...
legend(0.7, 1, legend=c("plant", "animal"), col=c("black", "blue"), pch=20, cex=1)
abline(h=0, v=0)
...
ey <- 0.2         # probability of extinction - animal
tmax <- 50        # maximum number of timesteps
DR <- 0.20        # fraction of patches destroyed at the start of restoration
...
dynamicsR <- function(cx, cy, ex, ey, tmax, DR, dR) {
  ...
  Pxrt[1, ] <- Pxdt[Di, ]
  pyrt <- matrix(0, nrow=length(r), ncol=length(t))    # fraction of occupied sites as function of time and R
  pyrt[1, ] <- pydt[Di, ]
  Pyrt <- matrix(0, nrow=length(r), ncol=length(t))    # fraction of occupied sites out of non-destroyed sites as function of time and R
  Pyrt[1, ] <- Pydt[Di, ]
  ...
  for(k in 2:length(r)){
    ...
    n_occ <- 0  # number of destroyed patches with x neighbours
    n_noc <- 0  # number of destroyed patches with empty neighbours only
    d_occ <- matrix(NA,nrow=sum(xreq[ , ,k]==-1),ncol=2) # matrix with coordinates of destroyed patches with x neighbours
    d_noc <- matrix(NA,nrow=sum(xreq[ , ,k]==-1),ncol=2) # matrix with coordinates of destroyed patches with empty neighbours only
      ...
      for(j in 1:n){
        ...
        if(xreq[i,j,k]!=-1) { next }
          ...
          for(h in 1:4){
            ...
            if(i_n < 1 || i_n > n || j_n < 1 || j_n > n) { next } # neighbour outside of grid
            if(xreq[i_n,j_n,k]==1 || xreq[i_n,j_n,k]==2) {
              n_occ <- n_occ+1
              d_occ[n_occ,1] <- i
              d_occ[n_occ,2] <- j
              n1 <- 1
    ...
    # restore patches adjecent to x-occupied patches first
    if(n_occ<dn) {
      if(n_occ>0){
        for(h in 1:n_occ) {
          i_d <- d_occ[h,1]
    ...
    # grid at t=0 for d[k]
    xt[ , ,1] <- xreq[ , ,k]
    pxrt[k,1] <- (sum(xt[ , , 1]==1)+sum(xt[ , , 1]==2))/(n*n)
    Pxrt[k,1] <- (sum(xt[ , , 1]==1)+sum(xt[ , , 1]==2))/(n*n-r[k])
    pyrt[k,1] <- sum(xt[ , , 1]==2)/(n*n)
    Pyrt[k,1] <- sum(xt[ , , 1]==2)/(n*n-r[k])
    ...
    for(g in 2:length(t)){
      ...
      pxrt[k,g] <- (sum(xt[ , , g]==1)+sum(xt[ , , g]==2))/(n*n)
      Pxrt[k,g] <- (sum(xt[ , , g]==1)+sum(xt[ , , g]==2))/(n*n-r[k])
      pyrt[k,g] <- sum(xt[ , , g]==2)/(n*n)
      Pyrt[k,g] <- sum(xt[ , , g]==2)/(n*n-r[k])
    }
    xreq[ , ,k] <- xt[ , ,length(t)]
  }
...
outR <- dynamicsR(cx, cy, ex, ey, tmax, DR, dR)   # habitat restoration output
...
legend(0.7, 1, legend=c("plant", "animal"), col=c("black", "blue"), pch=c(20,1), cex=1)
abline(h=0, v=0)

for(k in 1:length(R)){
  image(x=1:n, y=1:n, z=t(apply(xreq[ , ,k], 2, rev)), main=paste("D =", R[k]), xlab="", ylab="", zlim=c(-1,2),
        col=gray.colors(n=4, start = 0, end = 1, rev = FALSE))
  legend(grconvertX(1, "device"), grconvertY(1, "device"), 
         c("destroyed", "empty", "occupied-p", "occupied-p&a"), fill = gray.colors(n=4, start = 0, end = 1, rev = FALSE), xpd = NA)
}
...
Pyr <- matrix(0, nrow=nr, ncol=length(R))
for(i in 1:nr){
  outR <- dynamicsR(cx, cy, ex, ey, tmax, DR, dR)
  pxrt <- outR$pxrt
  Pxrt <- outR$Pxrt
  pxr[i, ] <- pxrt[ ,tmax+1]
  Pxr[i, ] <- Pxrt[ ,tmax+1]
