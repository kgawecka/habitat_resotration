...
# pairwise competition spatially explicit model
# non-random habitat resotration - x-adjacent method
#                                - sites adjecent to occupied by superior competitor restored first, 
#                                  then sites adjecent to occupied by inferior competitor restored, 
#                                  then empty restored

# species:
# x - superior competitor
# y - inferior competitor
...
cx <- 0.2         # probablity of colonisation - x
ex <- 0.2         # probability of extinction - x
cy <- 0.9         # probablity of colonisation - y
ey <- 0.2         # probability of extinction - y
px0 <- 0.2        # initial fraction of occupied sites - x
py0 <- 0.16       # initial fraction of occupied sites - y
n <- 100          # grid size
...
dynamicsD <- function(cx, cy, ex, ey, px0, py0, n, tmax, dD) {
  ...
  for(k in 1:length(d)){
    ...
    if(k>1){
      ...
      # grid at t=0 for d[k]
      xt[ , ,1] <- xdeq[ , ,k]
      pxdt[k,1] <- sum(xt[ , , 1]==1)/(n*n)
      Pxdt[k,1] <- sum(xt[ , , 1]==1)/(n*n-d[k])
    ...
    for(g in 2:length(t)){
      ...
      pxdt[k,g] <- sum(xt[ , , g]==1)/(n*n)
      Pxdt[k,g] <- sum(xt[ , , g]==1)/(n*n-d[k])
...
legend(0.7, 1, legend=c("x", "y"), col=c("black", "blue"), pch=20, cex=1)
abline(h=0, v=0)
...
dynamicsR <- function(cx, cy, ex, ey, tmax, DR, dR) {
  ...
  for(k in 2:length(r)){
    ...
    # restore patches adjecent to x-occupied patches first, then y-occupied, then empty
    if(n_x<dn) {
      if(n_x>0) {
        for(h in 1:n_x) {
          i_d <- d_x[h,1]
      ...
      if(n_y>=(dn-n_x)) {
        ...
        rp <- sample(1:n_e,(dn-n_x-n_y))
        for(h in 1:(dn-n_x-n_y)) {
          i_d <- d_e[rp[h],1]
          j_d <- d_e[rp[h],2]
          xreq[i_d,j_d,k] <- 0
        }
    ...
    # grid at t=0 for d[k]
    xt[ , ,1] <- xreq[ , ,k]
    pxrt[k,1] <- sum(xt[ , , 1]==1)/(n*n)
    Pxrt[k,1] <- sum(xt[ , , 1]==1)/(n*n-r[k])
    ...
    for(g in 2:length(t)){
      ...
      pxrt[k,g] <- sum(xt[ , , g]==1)/(n*n)
      Pxrt[k,g] <- sum(xt[ , , g]==1)/(n*n-r[k])
...
legend(0.7, 1, legend=c("x", "y"), col=c("black", "blue"), pch=c(20,1), cex=1)
abline(h=0, v=0)
...
for(k in 1:length(R)){
  ...
  legend(grconvertX(1, "device"), grconvertY(1, "device"), 
         c("destroyed", "empty", "occupied-x", "occupied-y"), fill = gray.colors(n=4, start = 0, end = 1, rev = FALSE), xpd = NA)
}
