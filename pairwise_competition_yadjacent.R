...
#                                - sites adjecent to occupied by inferior competitor restored first, 
#                                  then sites adjecent to occupied by superior competitor restored, 
#                                  then empty restored
...
n <- 100          # grid size
tmax <- 100        # maximum number of timesteps
...
dynamicsR <- function(cx, cy, ex, ey, tmax, DR, dR) {
  ...
  for(k in 2:length(r)){
    ...
    # restore patches adjecent to y-occupied patches only
    if(n_y>=dn) {
      rp <- sample(1:n_y,dn)
      for(h in 1:dn) {
        i_d <- d_y[rp[h],1]
