...
dynamicsR <- function(cx, cy, cz, ex, ey, ez, mu1, mu2, psi1, psi2, tmax, DR, dR) {
  ...
  for(k in 2:length(r)){
      ...
      for(j in 1:n){
        ...
        if(xreq[i,j,k]!=-1) { next }
          ...
          for(h in 1:4){
            ...
            if(i_n < 1 || i_n > n || j_n < 1 || j_n > n) { next } # neighbour outside of grid
            if(xreq[i_n,j_n,k]==3 || xreq[i_n,j_n,k]==5 || xreq[i_n,j_n,k]==6 || xreq[i_n,j_n,k]==7) {
              n_z <- n_z+1
              d_z[n_z,1] <- i
              d_z[n_z,2] <- j
              n1 <- 1
              break }               # if neighbour occupied by z, move to next patch
            else if(xreq[i_n,j_n,k]==2 || xreq[i_n,j_n,k]==4) {
              n2 <- 1 }
    ...
    # restore patches adjecent to z-occupied patches first, then y-occupied, then x-occupied, then empty
    if(n_z<dn) {
      if(n_z>0) {
        for(h in 1:n_z) {
          i_d <- d_z[h,1]
      ...
      if(n_y>=(dn-n_z)) {
        ...
        if(n_x>=(dn-n_z-n_y)) {
          rp <- sample(1:n_x,(dn-n_z-n_y))
          for(h in 1:(dn-n_z-n_y)) {
            i_d <- d_x[rp[h],1]
            j_d <- d_x[rp[h],2]
            xreq[i_d,j_d,k] <- 0
          }
        }
        else if(n_x<(dn-n_z-n_y)) {
          if(n_x>0) {
            for(h in 1:n_x) {
              i_d <- d_x[h,1]
              j_d <- d_x[h,2]
          ...
          rp <- sample(1:n_e,(dn-n_z-n_y-n_x))
          for(h in 1:(dn-n_z-n_y-n_x)) {
            i_d <- d_e[rp[h],1]
            j_d <- d_e[rp[h],2]
            xreq[i_d,j_d,k] <- 0
          }
