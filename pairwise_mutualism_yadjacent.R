...
#                                - sites adjecent to occupied by animal restored first, 
#                                  then sites adjecent to occupied by plant restored, 
#                                  then empty restored
...
dynamicsR <- function(cx, cy, ex, ey, tmax, DR, dR) {
  ...
  for(k in 2:length(r)){
    ...
    n_y <- 0  # number of destroyed patches with x&y neighbours only
    ...
    d_y <- matrix(NA,nrow=sum(xreq[ , ,k]==-1),ncol=2) # matrix with coordinates of destroyed patches with x&y neighbours
        ...
        if(xreq[i,j,k]!=-1) { next }
          ...
          for(h in 1:4){
            ...
            if(xreq[i_n,j_n,k]==2) {
              ...
              d_y[n_y,2] <- j
              n2 <- 1
              break }               # if neighbour occupied by x&y, move to next patch
    ...
    # restore patches adjecent to x&y-occupied patches only
    if(n_y>=dn) {
      rp <- sample(1:n_y,dn)
      for(h in 1:dn) {
        i_d <- d_y[rp[h],1]
    ...
    # restore patches adjecent to x&y-occupied patches first, then x-occupied, then empty
    if(n_y<dn) {
      if(n_y>0) {
        for(h in 1:n_y) {
          i_d <- d_y[h,1]
